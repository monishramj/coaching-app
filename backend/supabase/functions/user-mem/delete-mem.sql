...
CREATE FUNCTION delete_least_accessed_memories(p_user_id UUID, p_max_count INT)
RETURNS VOID AS $$
BEGIN
 -- Delete from table user-memories
  DELETE FROM "user-memories"
  ...
  WHERE memory_id IN (
    ...
    -- aka its the same as the user who has hit memory capacity
    ...
    LIMIT GREATEST((SELECT COUNT(*) FROM "user-memories" WHERE user_id = p_user_id) - p_max_count, 0)
  );
END;
$$ LANGUAGE plpgsql;